...
public abstract class DefaultCartService implements CartService {
    ...
    @Override
    public FindCartEntries.Result findCartEntriesWayTwo(FindCartEntries query) {
        if (findCartInternal(query.getCartId()) != null) {
            return FindCartEntries.Result.success(findCartEntriesInternal(query.getCartId()));
        }

        return FindCartEntries.Result.cartNotFound();
    }
}
